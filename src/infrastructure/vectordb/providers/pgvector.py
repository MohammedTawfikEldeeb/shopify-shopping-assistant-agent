...
from ..enum import (DistanceMetric, PgVectorTableSchemeEnums, 
...
class PGVectorProvider(VectorDBInterface):
    ...
    def __init__(self, db_client, default_vector_size: int = 1536,
    ...
    async def connect(self):
        async with self.db_client() as session:
            try:
                ...
                self.logger.warning("Vector extension setup: {}", str(e))
                try:
    ...
    async def create_collection(self, collection_name: str,
            ...
            async with self.db_client() as session:
                async with session.begin():
                    create_sql = sql_text(
                        ...
                        f'{PgVectorTableSchemeEnums.CHUNK_ID.value} uuid NULL UNIQUE'
                        ')'
                    )
        ...
        # migrate chunk_id to uuid if needed, and add unique constraint
        async with self.db_client() as session:
            async with session.begin():
                check_col_sql = sql_text(f"""
                    SELECT data_type FROM information_schema.columns
                    WHERE table_name = :table AND column_name = :col
                """)
                result = await session.execute(check_col_sql, {"table": collection_name, "col": PgVectorTableSchemeEnums.CHUNK_ID.value})
                row = result.scalar_one_or_none()
                if row and row.lower() in ('integer', 'bigint'):
                    self.logger.info(f"Migrating {collection_name}.chunk_id to uuid")
                    await session.execute(sql_text(
                        f'ALTER TABLE {collection_name} ALTER COLUMN {PgVectorTableSchemeEnums.CHUNK_ID.value} TYPE uuid USING NULL'
                    ))

                check_uniq_sql = sql_text(f"""
                    SELECT 1 FROM pg_indexes
                    WHERE tablename = :table AND indexdef ILIKE '%unique%'
                    AND indexdef ILIKE '%chunk_id%'
                """)
                has_unique = await session.execute(check_uniq_sql, {"table": collection_name})
                if not has_unique.scalar_one_or_none():
                    self.logger.info(f"Adding unique constraint on {collection_name}.chunk_id")
                    try:
                        await session.execute(sql_text(
                            f'ALTER TABLE {collection_name} ADD CONSTRAINT {collection_name}_chunk_id_key UNIQUE ({PgVectorTableSchemeEnums.CHUNK_ID.value})'
                        ))
                    except Exception:
                        self.logger.warning(f"Could not add unique constraint on chunk_id (may have null duplicates)")
                await session.commit()
            ...
            async with session.begin():
                for i in range(0, len(texts), batch_size):
                    ...
                    batch_insert_sql = sql_text(f'INSERT INTO {collection_name} '
                                    ...
                                    f'ON CONFLICT ({PgVectorTableSchemeEnums.CHUNK_ID.value}) DO NOTHING')
    ...
    async def get_existing_chunk_ids(self, collection_name: str, chunk_ids: list) -> set:
        is_collection_existed = await self.is_collection_exists(collection_name=collection_name)
        if not is_collection_existed:
            return set()
        ...
        async with self.db_client() as session:
            async with session.begin():
                query = sql_text(f'SELECT {PgVectorTableSchemeEnums.CHUNK_ID.value} FROM {collection_name} '
                                f'WHERE {PgVectorTableSchemeEnums.CHUNK_ID.value} = ANY(:ids)')
                result = await session.execute(query, {"ids": chunk_ids})
    ...
    async def query(self, collection_name: str, query_vector: list, top_k: int) -> list:
        return await self.search_by_vector(collection_name, query_vector, top_k)